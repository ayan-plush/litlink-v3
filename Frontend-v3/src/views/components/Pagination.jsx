...
const Pagination = ({pageNumber, setPageNumber, totalItem, perPage, showItem}) => {
    ...
    const createButton = () => {
        ...
        for (let i = startPage; i < endPage; i++) {
            btns.push(
                <li onClick={()=> setPageNumber(i)} className={` ${pageNumber===i ? 'bg-indigo-500 shadow-lg text-white ':'bg-[#857a65] hover:bg-[#857a655c] text-[#fff2df] hover:text-white'} w-[33px] h-[33px] rounded-full flex justify-center items-center cursor-pointer `}>
                    {i}
                </li>
            )
